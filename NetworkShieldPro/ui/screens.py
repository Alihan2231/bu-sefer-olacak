...
class SpotifyARPApp:
    ...
    def __init__(self, root):
        ...
        # Uygulama için icon
        try:
            # SVG ikonu kullanılabilir olmadığından, window ikonu ayarlamayı geç
            pass
        except Exception:
            pass
    ...
    def start_periodic_scan(self, interval_hours=24):
        ...
        if not self.scanner.periodic_running:
            self.scanner.start_periodic_scan(interval_hours)
            self.status_label.config(text=f"Periyodik: {interval_hours}s")
            return True
        return False
    
    def stop_periodic_scan(self):
        """Periyodik taramayı durdurur"""
        if self.scanner.periodic_running:
...
class SettingsScreen(BaseScreen):
    ...
    def save_settings(self):
        ...
        try:
            ...
            # (Şu anda sadece gösterme amaçlı, gerçekte bir etkileri yok)
